/**
 * @file test_writer_file_cerr.cpp
 * @brief Unit tests for ramrod::WriterFileCerr (writes to file and stderr).
 */
...
// Verifies that WriterFileCerr constructs with a File and header() runs without throwing.
TEST(WriterFileCerrTest, ConstructionWithFile)
{
    fs::path path{(fs::temp_directory_path() / "ramrod_writerfilecerr_test.txt").lexically_normal()};
    if (fs::exists(path))
        fs::remove(path);
    ramrod::File file(path);
    ramrod::WriterFileCerr writer(file);
    EXPECT_NO_THROW(writer.header("\e[0m", "[ERROR]"));
    fs::remove(path);
}

// Verifies that data written via operator<< is present in the file.
TEST(WriterFileCerrTest, WriteToFileAndReadBack)
{
    fs::path path{(fs::temp_directory_path() / "ramrod_writerfilecerr_write.txt").lexically_normal()};
    if (fs::exists(path))
        fs::remove(path);
    {
        ramrod::File file(path);
...
TEST(WriterFileCerrTest, ClearTruncatesFile)
{
    fs::path path{(fs::temp_directory_path() / "ramrod_writerfilecerr_clear.txt").lexically_normal()};
    if (fs::exists(path))
        fs::remove(path);
    {
        ramrod::File file(path);
...
// Verifies that all operator<< overloads in WriterFileCerr (Writer) write without throwing.
TEST(WriterFileCerrTest, OperatorShiftAllOverloads)
{
    fs::path path{(fs::temp_directory_path() / "ramrod_writerfilecerr_operator_shift.txt").lexically_normal()};
    if (fs::exists(path))
        fs::remove(path);
    ramrod::File file(path);
    ramrod::WriterFileCerr writer(file);
